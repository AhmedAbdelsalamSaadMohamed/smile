...
exports.commentPostNotification =functions.firestore.document("posts/{postId}/{comments}/{commentId}").onCreate(
    async (snapshot) =>{
      ...
      // eslint-disable-next-line max-len
      await firestore().collection("posts").doc(postId).collection("comments")
          .where("owner", "not-in", [commentOwnerId, post.owner_id] )
          .get().then(
              async (value) => {
                for (const comm of value.docs) {
                  // eslint-disable-next-line max-len
                  await firestore().collection("users").doc(comm.data().owner).collection("notifications").add({
                    "action": "comment_post_commenter",
                    "user_id": commentOwnerId,
                    "post_comment_id": snapshot.id,
                    "time": snapshot.data().time,
                    "seen": false,
                  ...
                  const payload = {
                    notification: {
                      // eslint-disable-next-line max-len
                      title: commentOwner.name + " Comment on same post you are comment",
                      // eslint-disable-next-line max-len
      ...
      const payload = {
        notification: {
          title: commentOwner.name+" Comment on your post",
          // eslint-disable-next-line max-len
          body: text ? (text.length <= 100 ? text : text.substring(0, 97) + "...") : "",
          icon: commentOwner.profile_url,
          click_action: "FLUTTER_NOTIFICATION_CLICK",
