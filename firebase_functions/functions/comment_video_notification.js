...
exports.commentVideoNotificationFun =functions.firestore.document("videos/{videoId}/{comments}/{commentId}").onCreate(
    async (snapshot) =>{
      const text = snapshot.data().text;
      const commentOwnerId = snapshot.data().owner;
      const videoId = snapshot.data().video_id;
      ...
      const commentOwner = await (firestore().collection("users").doc(commentOwnerId).get()).then(
        ...
        // eslint-disable-next-line max-len
      const video = await firestore().collection("videos").doc(videoId).get().then((value)=>{
        return value.data();
      });
        // post owner notify
        // eslint-disable-next-line max-len
      ...
      // eslint-disable-next-line max-len
      await firestore().collection("videos").doc(videoId).collection("comments")
      ...
      const payload = {
        ...
        // eslint-disable-next-line max-len
      await firestore().collection("videos").doc(videoId).get().then((video)=>{
        // eslint-disable-next-line max-len
